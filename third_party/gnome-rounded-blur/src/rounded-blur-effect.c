               ...
               ClutterEffectPaintFlags  flags)
{
  ...
  switch (self->mode)
    ...
    case GB_BLUR_MODE_BACKGROUND:
      /*
       * The background framebuffer sits below the popup as a separate stage
       * actor. Re-sampling it for damage caused by a sibling (for example an
       * hourly drag or a ScrollView update) can read pixels from the previous
       * composite and blend the blur into itself. The result is a dark damage
       * rectangle that grows while interacting with the popup.
       *
       * Refresh the snapshot when this actor itself is dirty (movement,
       * resize, opening animation or an explicitly queued effect repaint),
       * but reuse the completed blur for sibling-only damage.
       */
      return actor_dirty || !blur_cached;
    }
